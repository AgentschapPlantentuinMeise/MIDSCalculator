...
server <- function(input, output) {
  ...
  midssum <- reactive({
    gbif_dataset_mids() %>% group_by(mids_level) %>% summarise(n = n(), percentage = n()/nrow(.)*100)
  })
  ...
  output$midsplot<-renderPlot({
    ...
    geom_text(data = midssum(), aes(y = n + sum(n)*0.02, label = paste0(n, " (", round((n/sum(n))*100),"%)")))
  })
  ...
  #records table with mids levels
